...
// RICARDO:	Este programa funciona con el código
//			C:\Ricardo\ITESM\PIC\USB\USB_HIDDevice\HID_Firmware\hidterm\USBHID_GenIO.hex
//			descargado en el "kit" de Cypress CY3654.


//----------------------------------------------
#define RICH_VENDOR_ID			0x0000
#define RICH_USBHID_GENIO_ID	0x2011

#define INPUT_REPORT_SIZE	2
...
int LeerReporte(void)
{
	DWORD BytesRead = 0;
	DWORD BytesWritten = 0;
	int status = 0;

	memset(&reporteEntrada, 0, INPUT_REPORT_SIZE);
	...
	{
		printf("Buffer: %02X %02X %02X\n", reporteEntrada[0], reporteEntrada[1], reporteEntrada[2]);
	}
...
void PrepararJuego(void)
{
	...
	do
	{
		...
		case '1':
			...
			reporteSalida[1] = 0x80;
			reporteSalida[2] = 0x00;

			status = EscribirReporte();
			status = LeerReporte();

			if(reporteEntrada[1] ==  SIGUIENTE)
			{
				turno = 1;
			}
			break;
...
void ContinuarJuego(void)
{
	...
	do
	{
		printf("Jugador %d, elija un movimiento: \n", turno);
	...
	if(reporteEntrada[1] == SIGUIENTE)
	{
		turno = reporteEntrada[2];
	}
	else if(reporteEntrada[1] == TERMINA)
	{
		...
		else
		{
			...
			if(reporteEntrada[2] == 1)
			{
				count1++;
			}
			else if(reporteEntrada[2] == 2)
			{
				count2++;
			}
		}
...
void main ()
{
	...
	{
		printf("Vamos bien\n");
		while (!terminaAbruptaEInstantaneamenteElPrograma) 
		{
			Touch_Device();
			Sleep(500);
		}
